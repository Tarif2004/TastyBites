import React, { useState } from "react";
import { googleAuth } from "../services/api";

const GoogleLoginButton = ({ onSuccess, roleTarget = "user", buttonText = "Sign in with Google" }) => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const handleSimulatedGoogleAuth = async () => {
    try {
      setLoading(true);
      setError("");

      // Prompt or provide standard Google authentication fallback
      const userEmail = prompt(
        "Enter your Google Account email for fast Google Sign-In:",
        "user@gmail.com"
      );

      if (!userEmail) {
        setLoading(false);
        return;
      }

      const userName = userEmail.split("@")[0];

      const res = await googleAuth({
        email: userEmail,
        name: userName.charAt(0).toUpperCase() + userName.slice(1),
        googleId: `google_${Date.now()}`,
        targetRole: roleTarget,
      });

      if (res.token) {
        localStorage.setItem("token", res.token);
      }

      if (onSuccess) {
        onSuccess(res);
      }
    } catch (err) {
      setError(err.message || "Google Sign-In failed");
    } finally {
      setLoading(false);
    }
  };

  return (
    <div>
      {error && (
        <div className="text-xs text-rose-600 mb-2 font-medium bg-rose-50 p-2 rounded">
          {error}
        </div>
      )}
      <button
        type="button"
        onClick={handleSimulatedGoogleAuth}
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-300 rounded-xl bg-white hover:bg-slate-50 text-slate-700 font-semibold text-sm shadow-sm transition duration-200"
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
          />
        </svg>
        <span>{loading ? "Authenticating..." : buttonText}</span>
      </button>
    </div>
  );
};

export default GoogleLoginButton;
